...
func WriteFileAtomic(path string, data []byte, perm os.FileMode) error {
	dir := filepath.Dir(path)
	   if err := os.MkdirAll(dir, 0o755); err != nil {
		   ...
		   fmt.Printf("[Survival] CRITICAL: Failed to create directory: %v\n", err)
	...
	// Using a hidden prefix (.tmp-) to avoid issues with some tools
	   ...
	   if err != nil {
		   fmt.Printf("[Survival] CRITICAL: Failed to create temp file: %v\n", err)
	...
	// Note: Original file is untouched at this point
	   if _, err := tmpFile.Write(data); err != nil {
		   fmt.Printf("[Survival] CRITICAL: Failed to write temp file: %v\n", err)
	...
	// Essential for SD cards, eMMC, and other flash storage on edge devices.
	   if err := tmpFile.Sync(); err != nil {
		   fmt.Printf("[Survival] CRITICAL: Failed to sync temp file: %v\n", err)
		   return fmt.Errorf("failed to sync temp file: %w", err)
	   }

	// Set file permissions before closing
	   if err := tmpFile.Chmod(perm); err != nil {
		   fmt.Printf("[Survival] CRITICAL: Failed to set permissions: %v\n", err)
		   return fmt.Errorf("failed to set permissions: %w", err)
	   }

	// Close file before rename (required on Windows)
	   if err := tmpFile.Close(); err != nil {
		   fmt.Printf("[Survival] CRITICAL: Failed to close temp file: %v\n", err)
	...
	// On Windows: Rename() is atomic for files
	   if err := os.Rename(tmpPath, path); err != nil {
		   fmt.Printf("[Survival] CRITICAL: Failed to rename temp file: %v\n", err)
	...
	// This prevents the renamed file from disappearing after a crash
	   if dirFile, err := os.Open(dir); err == nil {
		   ...
		   fmt.Printf("[Survival] CRITICAL: Failed to sync directory: %v\n", err)
	   }
