...
func DownloadToFile(ctx context.Context, client *http.Client, req *http.Request, maxBytes int64) (string, error) {
	...
	logger.DebugCF("download", "Starting download", map[string]any{
	   ...
	   if err != nil {
		   logger.ErrorCF("download", "request failed", map[string]any{"error": err.Error()})
		   ...
		   for i := 0; i < 2; i++ {
			   ...
			   logger.WarnCF("download", "retrying request", map[string]any{"attempt": i + 2, "error": err.Error()})
		   }
		   if err != nil {
